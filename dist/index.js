...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Resource access failed for ${uri}: ${errorMessage}`, { uri, originalError: errorMessage });
    }
});
/**
 * Execute a tool
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (!executor) {
        ...
        throw new McpError(ErrorCode.MethodNotFound, `Unknown tool: ${name}. Available tools include: ${availableTools}...`);
    }
    ...
    try {
        ...
        if (error && typeof error === 'object' && 'response' in error) {
            ...
            throw new McpError(ErrorCode.InternalError, `Ambari API Error: ${httpError.message ?? 'Unknown error'}`, {
        ...
        // Handle other errors
        const errorMessage = error instanceof Error ? error.message : String(error);
        throw new McpError(ErrorCode.InternalError, `Tool execution failed for ${name}: ${errorMessage}`, { tool: name, originalError: errorMessage });
    }
});
// ============================================================================
// Server Startup
