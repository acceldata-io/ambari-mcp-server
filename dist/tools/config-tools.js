...
// Tool Executors
// ============================================================================
export const configToolExecutors = {
    ...
    ambari_config_search: async (args) => {
        ...
        const serviceFilter = args['serviceFilter'];
    ...
    ambari_config_dump: async (args) => {
        ...
        if (configType) {
            ...
            // Apply filter
            if (filterLower) {
                properties = Object.fromEntries(Object.entries(properties).filter(([k]) => k.toLowerCase().includes(filterLower) || configType.toLowerCase().includes(filterLower)));
            }
            const lines = [
        ...
        if (serviceFilter)
            header.push(`service_filter='${serviceFilter}'`);
        if (filter)
