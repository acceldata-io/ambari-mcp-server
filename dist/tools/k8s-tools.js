...
import { executeInPod, executeInPods, getAmbariPods, isK8sConfigured, getK8sStatus, isKubectlAvailable, } from '../k8s-client.js';
 ...
 */
async function resolvePods(podsArg, namespace) {
 ...
 */
async function getPodHostnameMap(pods) {
    const map = {};
    const results = await executeInPods(pods.map(p => p.name), 'hostname -f 2>/dev/null || hostname', { concurrency: 10 });
...
export const k8sToolExecutors = {
    ...
    ambari_k8s_list_pods: async (args) => {
        if (!isK8sConfigured()) {
            throw new McpError(ErrorCode.InvalidRequest, 'Kubernetes mode is not configured. Please set K8S_POD_LABEL_SELECTOR in your .env file.');
        }
    ...
    ambari_k8s_restart_agent_and_wait: async (args) => {
        ...
        const podsArg = args['pods'];
        const namespace = args['namespace'];
        const waitTimeSeconds = args['waitTimeSeconds'] || 60;
