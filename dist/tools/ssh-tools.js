...
import { executeRemoteCommandOnHosts, isSshConfigured, getSshStatus } from '../ssh-client.js';
 ...
 */
async function getAllClusterHosts() {
 ...
 */
async function resolveHosts(hostsArg) {
...
export const sshToolExecutors = {
    ...
    ambari_ssh_restart_agent: async (args) => {
        if (!isSshConfigured()) {
            throw new McpError(ErrorCode.InvalidRequest, 'SSH is not configured. Please set SSH_PRIVATE_KEY_PATH in your .env file.');
        }
        const hostsArg = args['hosts'];
