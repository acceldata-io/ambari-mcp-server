...
// Tool Executors
// ============================================================================
export const serviceToolExecutors = {
    ...
    ambari_services_getservice: async (args) => {
        ...
        if (args['fields'])
            params['fields'] = args['fields'];
        const data = await ambariGet(`/clusters/${clusterName}/services/${serviceName}`, params);
    ...
    ambari_services_startservice: async (args) => {
        const clusterName = args['clusterName'] || await getClusterName();
        const serviceName = args['serviceName'];
        const context = args['context'] || 'Start service via MCP';
        // Check current state first
        const current = await ambariGet(`/clusters/${clusterName}/services/${serviceName}`);
        ...
        const data = await ambariPut(`/clusters/${clusterName}/services/${serviceName}`, body);
        return {
            status: 'started',
    ...
    ambari_services_stopservice: async (args) => {
        const clusterName = args['clusterName'] || await getClusterName();
        const serviceName = args['serviceName'];
        const context = args['context'] || 'Stop service via MCP';
        // Check current state first
        const current = await ambariGet(`/clusters/${clusterName}/services/${serviceName}`);
        const currentState = current.ServiceInfo?.state;
    ...
    ambari_services_restartservice: async (args) => {
        ...
        const context = args['context'] || 'Restart service via MCP';
        ...
        const stopData = await ambariPut(`/clusters/${clusterName}/services/${serviceName}`, stopBody);
        ...
        if (waitForStop && stopRequestId) {
            ...
            while (attempts < maxAttempts) {
                const statusData = await ambariGet(`/clusters/${clusterName}/requests/${stopRequestId}`);
        ...
        const startData = await ambariPut(`/clusters/${clusterName}/services/${serviceName}`, startBody);
        return {
            status: 'restarting',
            stopRequestId,
    ...
    ambari_services_startall: async (args) => {
        const clusterName = args['clusterName'] || await getClusterName();
        const context = args['context'] || 'Start All Services via MCP';
        const body = {
            RequestInfo: {
                context,
                operation_level: {
        ...
        const data = await ambariPut(`/clusters/${clusterName}/services`, body);
        return {
            status: 'starting',
            requestId: data.Requests?.id,
            requestStatus: data.Requests?.status,
    ...
    ambari_services_stopall: async (args) => {
        const clusterName = args['clusterName'] || await getClusterName();
        const context = args['context'] || 'Stop All Services via MCP';
        const body = {
            RequestInfo: {
                context,
                operation_level: {
    ...
    ambari_services_getstaleconfigs: async (args) => {
        ...
        const data = await ambariGet(`/clusters/${clusterName}/host_components`, params);
    ...
    ambari_services_runservicecheck: async (args) => {
        const clusterName = args['clusterName'] || await getClusterName();
        const serviceName = args['serviceName'];
        const context = args['context'] || `${serviceName} Service Check via MCP`;
        ...
        const data = await ambariPost(`/clusters/${clusterName}/requests`, body);
        return {
            status: 'submitted',
            serviceName,
            command,
