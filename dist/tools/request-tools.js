...
export const requestToolExecutors = {
    ambari_requests_getstatus: async (args) => {
        ...
        if (request.create_time)
            lines.push(`  Created: ${formatTimestamp(request.create_time)}`);
        if (request.start_time)
            lines.push(`  Started: ${formatTimestamp(request.start_time)}`);
        if (request.end_time)
            lines.push(`  Ended: ${formatTimestamp(request.end_time)}`);
    ...
    ambari_requests_getrecent: async (args) => {
        ...
        const data = await ambariGet(`/clusters/${clusterName}/requests`, params);
    ...
    ambari_requests_gettasks: async (args) => {
        ...
        const data = await ambariGet(`/clusters/${clusterName}/requests/${requestId}/tasks`, params);
...
//# sourceMappingURL=request-tools.js.map