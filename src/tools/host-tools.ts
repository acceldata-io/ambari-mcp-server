...
interface HostData {
  Hosts?: {
    ...
    last_agent_env?: {
      hostHealth?: {
        liveServices?: Array<{ name?: string; status?: string; desc?: string }>;
        activeJavaProcs?: unknown[];
      ...
      existingRepos?: unknown[];
      existingUsers?: unknown[];
      alternatives?: unknown[];
      stackFoldersAndFiles?: unknown[];
    };
  };
  ...
  kerberos_identities?: unknown[];
}

async function formatHostDetails(hostName: string, clusterName: string): Promise<{ summary: string; data: HostData }> {
  ...
  if (hostComponents.length > 0) {
    ...
    // Group by service
    const byService: Record<string, Array<{ name: string; state: string }>> = {};
...
export const hostToolExecutors: Record<string, (args: Record<string, unknown>) => Promise<unknown>> = {
  ...
  ambari_hosts_gethosts: async (args) => {
    ...
    const data = await ambariGet(`/clusters/${clusterName}/hosts`, params) as {
      items?: Array<{ Hosts?: { host_name?: string; host_status?: string; host_state?: string } }>;
    };
  ...
  ambari_hosts_getallhostdetails: async (args) => {
    ...
    const hostsData = await ambariGet(`/clusters/${clusterName}/hosts`, {
      ...
      items?: Array<{ Hosts?: { host_name?: string } }>;
    };

    const hosts = hostsData.items ?? [];
    const results: Array<{ hostName: string; summary: string; data: unknown }> = [];
  ...
  ambari_hosts_gethostcomponents: async (args) => {
    ...
    // Group by service
    const byService: Record<string, Array<{ name: string; state: string; stale: boolean }>> = {};
